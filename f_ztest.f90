...
!
      ...
      parameter ( maxn = 2, maxnz = 100, maxnrhs = 10 )
      integer rowind(maxnz), colptr(maxn)
      double complex  values(maxnz), b(maxn), berr(maxnrhs), B_rhs(maxn), X_sol(maxn), diagonals(maxn), det, logdet
...
! Call the linear equation solver
      ...
      call f_zGetDiagU(n, LUstruct, grid, diagonals)
      write(*,*)'diagonals = ',diagonals
      det = 1.d0
      logdet = 0.d0
      do i = 1, n
         det = det*diagonals(i)
         logdet = logdet +log(diagonals(i)*conjg(diagonals(i)))
      enddo
      write(*,*)'det = ',det
      write(*,*)'logdet 1 = ',log(det*conjg(det))
      write(*,*)'logdet 2 = ',logdet
